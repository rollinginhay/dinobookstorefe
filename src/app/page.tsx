...
export default function Home() {
  ...
  const { calculatePrice, campaigns, loading: campaignsLoading } = useCampaign(); // ✅ Để check discount từ campaign (PERCENTAGE_PRODUCT)
  const hasFeatured = featuredBooks.length > 0;
  // ✅ Chỉ lấy 10 cuốn cho sách nổi bật
  const displayFeatured = (hasFeatured ? featuredBooks : fallbackFeaturedBooks).slice(0, 10);
  
  // 🔄 Tạo infinite loop: duplicate items ở đầu và cuối
  const loopedBooks = displayFeatured.length > 0 
    ? [...displayFeatured, ...displayFeatured, ...displayFeatured]
    : [];

  // 🎠 Hàm scroll mượt với smooth animation
  const smoothScroll = (element: HTMLElement, target: number, duration: number = 600) => {
  ...
  // 🟢 Fetch TOÀN BỘ SÁCH
  async function fetchAllBooks(campaigns: Array<{
    id?: string | number;
    attributes?: {
      campaignType?: string;
      percentage?: number;
    ...
    try {
      ...
      if (books.length > 0) {
        ...
        books.forEach((book: Book) => {
          ...
          const hasCampaignDiscount = !!(book.originalPrice && book.originalPrice > book.price);
        ...
        console.log(`✅ [Featured] Tìm thấy ${booksWithDiscount.length} sách có giảm giá, hiển thị ${featured.length} cuốn`);
  ...
  useEffect(() => {
    async function fetchOrderDiscountCampaign() {
      try {
        const res = await fetch("http://localhost:8080/v1/campaigns?e=true");
        if (!res.ok) {
        ...
        console.log("📋 [Banner] Total campaigns:", rawCampaigns.length);
        const now = new Date();
        ...
        const orderCampaign = rawCampaigns.find((c: any) => {
          ...
          if (campaignType !== "PERCENTAGE_RECEIPT" && campaignType !== "PERCENTAGE_DISCOUNT") {
            return false;
          }
          ...
          if (startDate && endDate) {
            ...
            if (now < start || now > end) {
              console.log("⏰ [Banner] Campaign ngoài thời gian:", { start, end, now });
              return false;
            }
          }
        ...
        if (orderCampaign) {
          ...
          console.log("✅ [Banner] Found order discount campaign:", campaignData);
          setOrderDiscountCampaign(campaignData);
        } else {
          console.log("⚠️ [Banner] Không tìm thấy campaign PERCENTAGE_RECEIPT/PERCENTAGE_DISCOUNT active");
        }
      } catch (err) {
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="relative overflow-hidden bg-gradient-to-br from-red-700 via-red-800 to-red-900 rounded-2xl shadow-2xl">
              ...
              <div className="relative z-10 flex items-center justify-between px-6 md:px-8 lg:px-10 py-6 md:py-7">
                  ...
                  <div className="flex items-center gap-3 md:gap-4 flex-1 min-w-0">
                    ...
                    <div className="relative bg-yellow-400 px-4 md:px-5 py-2 md:py-2.5 rounded-xl shadow-lg flex-shrink-0">
                      ...
                      <div className="relative">
                        <div className="text-red-800 font-black text-xs md:text-sm mb-0.5 leading-none">GIẢM</div>
                        <div className="text-red-900 font-black text-base md:text-xl leading-none">-{orderDiscountCampaign.percentage}%</div>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-1 min-w-0">
                      {orderDiscountCampaign.maxDiscount > 0 && (
                        <span className="text-white font-semibold text-xs md:text-sm leading-tight">
                          Tối đa {orderDiscountCampaign.maxDiscount.toLocaleString('vi-VN')}₫
                        </span>
                      )}
                      <span className="text-white/90 font-medium text-xs md:text-sm leading-tight whitespace-nowrap">
                        Khi mua từ {orderDiscountCampaign.minTotal.toLocaleString('vi-VN')}₫
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {orderDiscountCampaign.startDate && orderDiscountCampaign.endDate && (
                  <div className="flex-shrink-0 text-right pl-5 md:pl-6 border-l-2 border-white/30 hidden md:block">
                    <p className="text-xs text-white/80 mb-1 uppercase tracking-wide">Thời gian</p>
                    <p className="text-sm md:text-base font-bold text-white">
                      {new Date(orderDiscountCampaign.startDate).toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit' })} - {new Date(orderDiscountCampaign.endDate).toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit' })}
                    </p>
                  </div>
                )}
              </div>
