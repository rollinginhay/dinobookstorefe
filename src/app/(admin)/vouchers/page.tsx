...
export default function VouchersPage() {
  ...
  const loadData = async () => {
    try {
      ...
      console.log("Fetched vouchers response:", res);
      ...
      const mapped = mapVoucherList(res);
  ...
  useEffect(() => {
    ...
    if (statusFilter !== "ALL") {
      filtered = filtered.filter((item) => {
        const status = getVoucherStatus(item.startDate, item.endDate);
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-lg font-semibold">Phiếu giảm giá</h1>
        ...
        {(searchTerm || statusFilter !== "ALL") && (
          <div className="flex items-center justify-between pt-2 border-t">
            <p className="text-xs text-gray-500">
              Tìm thấy {filteredData.length} phiếu giảm giá
            </p>
            <button
              onClick={() => {
                setSearchTerm("");
