...
export default function BookCard({ book }: BookCardProps) {
  ...
  const originalPrice = book.originalPrice || Math.round(book.price * (1 + discount / 100));
  ...
  return (
    <div className="group bg-white rounded-xl shadow-sm hover:shadow-2xl transition-all duration-300 overflow-hidden border border-gray-100 transform hover:-translate-y-2">
      {/* Image */}
  ...
  {/* Favorite Button */}
  <button 
    className={`absolute top-3 right-3 p-2 bg-white rounded-full shadow-lg hover:bg-gray-50 transition-all z-10 transform hover:scale-110 ${isFav ? 'text-red-500' : 'text-gray-400'}`}
  ...
  >
    <svg className="w-5 h-5" fill={isFav ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="p-4">
        {/* Title */}
        <Link href={`/san-pham/${book.id}`}>
          <h3 className="font-bold text-sm mb-2 line-clamp-2 text-gray-900 hover:text-blue-600 cursor-pointer transition-colors">
            {book.title}
          </h3>
        </Link>
        
        {/* Author */}
        <p className="text-gray-600 text-xs mb-3">Tác giả: <span className="font-medium">{book.author}</span></p>
        ...
        {soldCount > 0 && (
          <p className="text-xs text-gray-600 mb-3">
            Đã bán <span className="font-semibold text-orange-600">
